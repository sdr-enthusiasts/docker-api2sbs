...
It _should_ work with many other APIs that follow the de-facto ADSBExchange V2 standard, but the JSON key names appear to vary. To add a service that uses different keys, please edit the (`json_keys`)[scripts/json_keys] file and submit a PR. Please also update this README and the (`docker-compose.yml`)[docker-compose.yml] file to reflect and explain the change.
